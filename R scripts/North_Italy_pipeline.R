...
############################## CHOOSE REgion -> North Italy
Region <- c('Veneto','Friuli','Trentino','Lombardia')
...
V(net)$size <- sqrt(Vertex.df$Popolazione)/100
V(net)$size
...
plot(net,vertex.size=V(net)$size, edge.arrow.size=0.05,vertex.label=V(net)$label,
     ...
     main=paste('North Italy. Clustering coeff:', cluster_tr)) 
...
matplot(x = times, y = df_sim, type = "l",
        xlab = "Time", ylab = "SEIR", main = paste("SEIR Model,North Italy. Start at:",name_id),
