...
#######################TRY PERCOLATION
df_percol <- df_part
df_percol[df_percol$Resid==df_percol$Dest, ]$Flux <- 0 
df_percol <- df_percol[df_percol$Flux>0,]
df_percol



Vertex.perc.df <- subset(Comune_Names,Codice_Istat %in% union(df_percol$Resid,df_percol$Dest))
Vertex.perc.df

############# ONLY TO ADD THIS MUNICIPALITY
Vertex.perc.df[nrow(Vertex.perc.df)+ 1,] <- list(13249,'Comunex','03','015',4000)
### 

net.p <- graph_from_data_frame(df_percol,directed=TRUE, vertices = Vertex.perc.df)
net.p
global_efficiency(net.p)


save_glob_eff<-c()
for (i in 1:500){
  df_percol <- df_percol[df_percol$Flux >i,]
  net.p <- graph_from_data_frame(df_percol,directed=TRUE)
  save_glob_eff <- c(save_glob_eff,global_efficiency(net.p))
}


Small_Flux <- 1:500
Glob_eff_df <- data.frame(Small_Flux,save_glob_eff)
Glob_eff_df

plot(Small_Flux,save_glob_eff)

write.csv2(Glob_eff_df,file = 'Global_Efficiency_North_It.csv')
...
## Add legend
legend('topright', c("Susceptible",'Exposed', "Infected", "Recovered"), pch = 1,cex=1, col = 1:4, bty = "n",y.intersp = 0.4)
...
## Add legend
legend(x=240,y=500,Top_comun$Denominazione,cex=1, col = 1:num_top_comun,pch=1, bty = "n",y.intersp = 0.2)
