...
hist(degree(net_an,mode=c('out')),breaks = 100)
...
hist(degree(net_an, mode = c('total')),breaks = 100, main = 'Degrees of Nodes (Municipality): Puglia.',
     xlab = 'Degrees')
...
#################################### SEE HOW IT GOES IN DIFFERENT CITIES
##show differences in Cityes...

Simulation_SEIR_pend_2 <- function(zone0_infected,zone0_id,
                                   beta,gamma,sigma,max_sim,OD, Pop){
...
df_list <- Simulation_SEIR_pend_2(zone0_infected,zone0_id,
                                           beta,gamma,sigma,max_sim,OD,Pop)
...
matplot(x = times, y = df_top_I, type = "l",ylim=c(0,1),
        xlab = "Time", ylab = "SEIR", main = paste("SEIR Model,",Region, '. Start at:',name_id),
        lwd = 2, lty = 1, bty = "l", col = 1:num_top_comun)

## Add legend
legend(length(times)-50, 0.95, Top_comun$Denominazione, col = 1:num_top_comun,pch = 1, bty = "n")
...
matplot(x = times, y = df_sim, type = "l",
        ...
        lwd = 2, lty = 1, bty = "l", col = 1:4)
## Add legend
legend('topright', c("Susceptible",'Exposed', "Infected", "Recovered"), pch = 1,cex=0.7, col = 1:4, bty = "n",y.intersp = 0.4)

#PLOT 2
times <- seq(1,max_sim+1,1)
matplot(x = times, y = df_top_I, type = "l",ylim=c(0,1),
        xlab = "Time", ylab = "Inf Rate", main = paste("Infection rate at different big cities"),
        lwd = 2, lty = 1, bty = "l", col = 1:num_top_comun)

## Add legend
legend(250,0.8,cex=0.7,y.intersp = 0.4, Top_comun$Denominazione, col = 1:num_top_comun,pch=1, bty = "n")
...
matplot(x = times, y = df_top_true_pop, type = "l",ylim=c(0,max(df_top_true_pop)),
        xlab = "Time", ylab = "N. Infected (1000)", main = paste("Infection rate at different big cities"),
        lwd = 2, lty = 1, bty = "l", col = 1:num_top_comun)

## Add legend
legend('topright',cex=1,y.intersp = 0.7, Top_comun$Denominazione, col = 1:num_top_comun,pch=1, bty = "n")
