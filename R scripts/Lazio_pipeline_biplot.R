...
plot(net,vertex.size=V(net)$size, edge.arrow.size=0.05,vertex.label=V(net)$label,
     ...
     main=paste('Region of',Region,'. Clustering coeff:', cluster_tr)) 
...
max_sim <- 250 # maximum number of day simulated
...
matplot(x = times, y = df_sim, type = "l",
        xlab = "Time", ylab = "SEIR",main = paste("SEIR Model:",Region),
        lwd = 2, lty = 1, bty = "l", col = 1:4)

text(max_sim/2,1.1,paste('Start at:',name_id))

## Add legend
legend('topleft', c("Susceptible",'Exposed', "Infected", "Recovered"), pch = 1,cex=1, col = 1:4, bty = "n",y.intersp = 0.7)
...
## Add legend
legend(x=50,y=900,Top_comun$Denominazione,cex=1, col = 1:num_top_comun,pch=1, bty = "n",y.intersp = 0.7)
